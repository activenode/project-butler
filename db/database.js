...
class ProjectDatabase {
   ...
   setupEmptyMaps() {
      ...
      this.autoGeneratedAliasesToIndex = new Map();
   ...
   async debug() {
      ...
      log("");
      this.autoGeneratedAliasesToIndex.forEach((key, value) => {
         log(
            `Auto Generated Alias [${value}] mapped to ${
               this.indexToProject.get(key).absPath
            }`
         );
      });
      log("");
   ...
   parseData(dataStruct) {
      this.setupEmptyMaps();
      this.nextUsableCounts = dataStruct.nextUsableCounts;
      if (!this.nextUsableCounts) {
         this.nextUsableCounts = [0];
      }
      ...
      Object.keys(dataStruct.projects.quickRef).forEach((uid, index) => {
         ...
         dataStruct.projects.quickRef[uid].autoGeneratedAliases.forEach(
            (alias) => this.autoGeneratedAliasesToIndex.set(alias, index)
         );
         projectCounter++;
      });
      this.nextUsableCounts?.length === 0 &&
         this.nextUsableCounts.push(projectCounter);
      dataStruct.nextUsableCounts = this.nextUsableCounts;
   ...
   async getProjectDetailsByUID(uid) {
      return this.load().then(({ projects: { quickRef } }) => {
         ...
         projectData.autoGeneratedAliases = quickRef[uid].autoGeneratedAliases;
         return projectData;
      });
   }
   ...
   async getExactProjectResultByAlias(sAlias) {
      ...
      const projectIndex =
         ...
         this.autoGeneratedAliasesToIndex.get(sAlias);
      const uid = this.indexToUid.get(projectIndex);
   ...
   findNextBestMatch(searchString) {
      return this.fetchAll().then((allProjects) => {
         ...
         const matchedProjects = allProjects.projects.filter(
            ({ absPath, aliases, autoGeneratedAliases }) => {
               const searchables = [
                  absPath,
                  ...aliases,
   ...
   findBestMatch(searchString) {
      ...
      return this.load().then(() => {
         if (
            this.aliasesToIndex.has(searchString) ||
            this.autoGeneratedAliasesToIndex.has(searchString)
         ) {
   ...
   async mapExistingAliasesWithProjectDirs(aliasesArray, options) {
      ...
      const aliasesInUse = aliasesArray
         .filter(
            (alias) =>
               this.aliasesToIndex.has(alias) ||
               this.autoGeneratedAliasesToIndex.has(alias)
         )
         .map((alias) => {
            const { absPath } = this.indexToProject.get(
               this.aliasesToIndex.get(alias) ||
                  this.autoGeneratedAliasesToIndex(alias)
            );
   ...
   async addProject(absPath, aliases) {
      ...
      return this.load(true).then((obj) => {
         let aliasesToWrite = [];
         let autoGeneratedAliasesToWrite = [];
         if (obj.projects.quickRef[uid]) {
            projectExistedBefore = true;
            aliasesToWrite = [].concat(obj.projects.quickRef[uid].aliases);
            autoGeneratedAliasesToWrite = [].concat(
               obj.projects.quickRef[uid].autoGeneratedAliases
            );
         } else {
            autoGeneratedAliasesToWrite = [].concat(
               this.getSetOfAutoAliases(parseInt(this.nextUsableCounts.pop()))
            );
         }

         log(this.nextUsableCounts);
         ...
         obj.projects.quickRef[uid] = {
            ...
            autoGeneratedAliases: autoGeneratedAliasesToWrite,
         };
   ...
   async removeByAliases(aliases, bDeleteAll) {
      return this.load().then((obj) => {
         const existingAliases = (Array.isArray(aliases) ? aliases : []).filter(
            (sAlias) => {
               return (
                  this.aliasesToIndex.has(sAlias) ||
                  this.autoGeneratedAliasesToIndex.has(sAlias)
               );
            }
         );
         ...
         const affectedUidsArr = Array.from(
            existingAliases.reduce((oSet, sAlias) => {
               ...
               log(
                  this.aliasesToIndex.get(sAlias) ||
                     this.autoGeneratedAliasesToIndex(s)
               );
            ...
            if (obj.projects.quickRef[uid]) {
               if (bDeleteAll) {
                  ...
                  if (obj.projects.quickRef[uid].aliases.length === 0) {
                     this.nextUsableCounts.push(
                        obj.projects.quickRef[uid].autoGeneratedAliases.find(
                           (autoAlias) => parseInt(autoAlias)
                        )
                     );
                     delete obj.projects.quickRef[uid];
                  }
